...
<template>
  ...
  <section v-if="points.length" class="mt-5 pt-4 border-t border-zinc-200/60 animate-fade-in">
    <div class="flex items-center gap-1.5 mb-3">
      ...
      <span class="text-[10px] font-black text-zinc-400 uppercase tracking-widest">{{ hasStudy ? '课前语法预习' : '本课语法地图' }}</span>
      <InfoTip
        :text="hasStudy
    ...
    <div class="space-y-3">
      <div v-for="(p, i) in points" :key="i">
        <h3 class="text-xs font-bold text-zinc-700 mb-1.5 leading-relaxed">{{ p.point }}</h3>
        <div class="flex flex-wrap gap-1.5">
          <component
            :is="hasStudy ? 'button' : 'span'"
            v-for="ref in p.refs"
            :key="ref.unit"
            class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold border bg-zinc-100 text-zinc-700 border-zinc-200/70"
            :class="hasStudy ? 'cursor-pointer hover:bg-zinc-100 hover:border-zinc-400 hover:shadow-sm transition-all' : ''"
    ...
    <button
      v-if="hasStudy"
      @click="studyRef?.open()"
      class="mt-3 w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-zinc-900 text-white hover:bg-zinc-900 shadow-md shadow-zinc-900/15 transition-all"
    >
