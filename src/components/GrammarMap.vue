...
<template>
  ...
  <section v-if="points.length" class="mt-5 pt-4 border-t border-slate-200/60 animate-fade-in">
    ...
    <button
      v-if="hasStudy"
      @click="studyRef?.open()"
      class="mt-3 w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-amber-500 text-white hover:bg-amber-600 shadow-md shadow-amber-500/20 transition-all"
    >
    ...
    <p class="mt-3 text-[10px] text-slate-400 leading-relaxed">
      <template v-if="hasStudy">听课前先花 5 分钟学完这里，听课文时你会「对上号」。讲解与练习为本站原创编写；想系统深入，可翻《{{ book.name }}》（{{ book.edition }}）对应 Unit。</template>
